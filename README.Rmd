...
By default, the database is named `ukb-schemas-YYYY-MM-DD.sqlite` and placed in the current working directory (`path = tempdir()` in the above example puts it in the current temporary directory instead). At time of compilation of this README (`r format(Sys.Date(), "%d %b %Y")`), the size of the .sqlite database file produced by `create_schema_db()` is approximately `r fsize`MB.

The second function, `load_schema_db()`, loads the tables from the database and stores them as tibbles in a named list:
...
* The UK Biobank data schemas are regularly updated as new data are added to the system. ukbschemas does not currently include a facility for updating the database; it is necessary at present to create a new database.
...
* Value types as described in http://biobank.ctsu.ox.ac.uk/crystal/help.cgi?cd=value_type are not included in the UKB schemas. See [Design notes](https://github.com/bjcairns/ukbschemas#design-notes), above.
